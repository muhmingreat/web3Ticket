...
const Page = ({ eventData, ticketsData }) => {
  ...
  return event ? (
    <div>
      <Head>
        <title>M M ticket  | {event.title}</title>
          ...
          <div className="w-full">
            <div className="flex flex-wrap justify-start items-center space-x-2 mt-4">
              ...
              {!event.minted ? (
                ...
                <span className="bg-cyan-600 text-white rounded-xl px-4">Closed</span>
            ...
            <div className="flex flex-col sm:flex-row justify-start sm:items-center my-4">
              <div className="flex justify-start items-center">
                <FaEthereum />
                <p className=" font-bold">{event.ticketCost.toFixed(3)} ETH </p>
              </div>
            ...
            <div className="flex justify-start items-center space-x-4 my-8">
              {event.endsAt > Date.now() && (
                <button onClick={()=> dispatch(setTicketModal('scale-100'))}
                ...
                <div className="flex justify-end items-center w-full">
                  <div className="flex justify-start items-center">
                    <span className="flex items-center">
                      <FaEthereum /> <span>{ticket.ticketCost.toFixed(3)}</span>
                    </span>
                    <BsDot size={30} />
...
export const getServerSideProps = async (context) => {
  ...
  const ticketsData = await getTickets(Number(id))
